...
describe("useCustomCompareEffect", () => {
    it("should run effect every time rerender, when compare fn return false", function () {
        ...
        const hook = renderHook(({ deps }: { deps: any[] }) => useCustomCompareEffect(fn, deps, () => false), { initialProps });
    ...
    it("should not run effect when compare return true", function () {
        ...
        const hook = renderHook(({ deps }: { deps: any[] }) => useCustomCompareEffect(fn, deps, equal), { initialProps });
        expect(fn).toHaveBeenCalledTimes(1);
        hook.rerender({
            deps: [2, { name: "chen" }],
        });
